...
(function () {
  ...
  function userId() {
    let id = localStorage.getItem('xiaoqi.agent.uid');
    if (!id) { id = `ps_${Date.now().toString(36)}_${Math.floor(Math.random() * 1e9).toString(36)}`; localStorage.setItem('xiaoqi.agent.uid', id); }
    return id;
  }
  ...
  function applyCollapse() {
    const collapsed = localStorage.getItem('xiaoqi.agent.collapsed') === '1';
  ...
  async function ensureLoaded() {
    applyCollapse();
    a.regionMode = localStorage.getItem('xiaoqi.agent.region') === '1';
    a.outpaintMode = false;
    localStorage.setItem('xiaoqi.agent.outpaint', '0');
    if (a.regionMode && a.outpaintMode) a.outpaintMode = false;   // 互斥
    // 恢复扩图方向勾选 + px 记忆
    try {
      const saved = JSON.parse(localStorage.getItem('xiaoqi.agent.outpaintDirs') || '{}');
    ...
    const savedPx = localStorage.getItem('xiaoqi.agent.outpaintPx');
    ...
    try {
      ...
      DX.ui.fillPicker(els.llmProvider, a.chatProviders.map((p) => ({ value: p.id, label: p.name || p.id })), localStorage.getItem('xiaoqi.agent.llmp'));
      DX.ui.fillPicker(els.imgProvider, a.imgProviders.map((p) => ({ value: p.id, label: p.name || p.id })), localStorage.getItem('xiaoqi.agent.imgp'));
      renderLlmModels(localStorage.getItem('xiaoqi.agent.llmm'));
      renderImgModels(localStorage.getItem('xiaoqi.agent.imgm'));
      a.loaded = true;
  ...
  async function sendOutpaint(msg, lp, ip) {
    ...
    // 记忆所选模型
    localStorage.setItem('xiaoqi.agent.llmp', lp.id);
    localStorage.setItem('xiaoqi.agent.llmm', DX.ui.pickerValue(els.llmModel));
    localStorage.setItem('xiaoqi.agent.imgp', ip.id);
    localStorage.setItem('xiaoqi.agent.imgm', DX.ui.pickerValue(els.imgModel));
  ...
  els.toggleModels.addEventListener('click', () => {
    const c = localStorage.getItem('xiaoqi.agent.collapsed') === '1';
    localStorage.setItem('xiaoqi.agent.collapsed', c ? '0' : '1');
    applyCollapse();
  });
  ...
  if (els.regionBtn) els.regionBtn.addEventListener('click', () => {
    ...
    localStorage.setItem('xiaoqi.agent.region', a.regionMode ? '1' : '0');
    localStorage.setItem('xiaoqi.agent.outpaint', a.outpaintMode ? '1' : '0');
    applyRegion();
  ...
  function saveOutpaintDirs() {
    localStorage.setItem('xiaoqi.agent.outpaintDirs', JSON.stringify({
      left: !!(els.dirLeft && els.dirLeft.checked),
      right: !!(els.dirRight && els.dirRight.checked),
      top: !!(els.dirTop && els.dirTop.checked),
      bottom: !!(els.dirBottom && els.dirBottom.checked),
  ...
  if (els.outpaintPx) els.outpaintPx.addEventListener('change', () => localStorage.setItem('xiaoqi.agent.outpaintPx', String(els.outpaintPx.value || 256)));
