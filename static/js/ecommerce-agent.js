(function(){
    'use strict';
    var state={platforms:[],providers:[],brands:[],templates:[],detailPlan:null,history:[],run:null,action:'create',timer:0,parseTimer:0,analysis:null,viewerIndex:0,viewerSources:[],viewerSourceIndex:0,viewerLoadToken:0,viewerTimer:0};
    ...
    function html(){
        ...
        '<section class="ec-section"><div class="ec-section-head"><i data-lucide="scan-search"></i><strong>选中素材</strong><span id="ecSelectedCount">0 个节点</span></div><div id="ecSelected" class="ec-selected"></div><div class="ec-actions"><button class="ec-btn" id="ecRefreshSelection">刷新选中内容</button><button class="ec-btn" id="ecCreateNode">放入 Agent 节点</button><button class="ec-btn primary" id="ecAnalyze">分析并规划</button></div><p class="ec-hint">Agent 只读取当前选中的图片、文字和参考素材，不会混入画布里的其他内容。</p><div id="ecAnalysis" class="ec-analysis"><div class="ec-empty">点击“分析并规划”，先查看 Agent 准备怎么做。</div></div></section>',
        '<section class="ec-section"><div class="ec-section-head"><i data-lucide="wand-sparkles"></i><strong>创作指令</strong><span>只读取当前选中内容</span></div><div id="ecActionChips" class="ec-action-chips"><button type="button" class="ec-action-chip active" data-ec-action="create">开始创作</button><button type="button" class="ec-action-chip" data-ec-action="variation">生成变体</button><button type="button" class="ec-action-chip" data-ec-action="background">换背景</button><button type="button" class="ec-action-chip" data-ec-action="expand">扩展画面</button><button type="button" class="ec-action-chip" data-ec-action="refine">局部优化</button><button type="button" class="ec-action-chip" data-ec-action="continue">继续创作</button></div><label class="ec-field"><span>告诉 Agent 你想怎么改</span><textarea id="ecRequest">根据选中的内容，生成 4 张风格统一但构图不同的创作方案</textarea></label><div id="ecParseSummary" class="ec-parse-summary"></div><div id="ecWarnings"></div>',
        ...
        '<div class="ec-row"><label class="ec-field"><span>图片 API</span><select id="ecProvider"></select></label><label class="ec-field"><span>图片模型</span><select id="ecModel"></select></label></div><label class="ec-check" style="margin-top:8px"><input id="ecAddText" type="checkbox" checked>保留可编辑文字层</label><label class="ec-check"><input id="ecStrictFidelity" type="checkbox" checked>严格保留主体、Logo 和原有文字</label></section>',
    ...
    function renderModels(){
        var p=state.providers.find(function(item){return item.id===one('#ecProvider').value})||state.providers[0];
        one('#ecModel').innerHTML=((p&&p.image_models)||[]).map(function(model){return '<option value="'+esc(model)+'">'+esc(model)+'</option>'}).join('')||'<option value="">没有图片模型</option>';
    }
    ...
    function renderTemplatePlan(){
        ...
        target.innerHTML='<div class="ec-template-plan-head"><strong>'+esc(p.template_name)+'</strong><span>'+Number(p.total_images)+' 张 · 不扣费预览</span></div><p class="ec-template-description">'+esc(p.template_description||'')+'</p><div class="ec-module-list">'+(p.modules||[]).map(function(m,i){return '<div class="ec-module-item"><b>'+String(i+1).padStart(2,'0')+'</b><span><strong>'+esc(m.module_name)+'</strong><small>'+esc(m.kind)+' · '+esc(m.purpose)+'</small></span><em>'+esc(m.text_hint)+'</em></div>'}).join('')+'</div><div class="ec-template-plan-actions"><button type="button" class="ec-btn primary" id="ecApplyTemplate">应用这个模板</button><span>应用后会按模块逐张生成，旧图片不会被覆盖。</span></div>';
    }
    function selectTemplate(id){
        ...
        api('/api/ecommerce-agent/detail-plan',{method:'POST',body:JSON.stringify({input_snapshot:source,request_text:one('#ecRequest').value,template_id:id})}).then(function(data){state.detailPlan=data.plan||null;renderTemplates();renderTemplatePlan();parseRequest()}).catch(function(e){one('#ecTemplatePlan').innerHTML='<div class="ec-warning">模板预览失败：'+esc(e.message)+'</div>'});
    }
    function renderBase(){
        one('#ecProvider').innerHTML=state.providers.map(function(p){return '<option value="'+esc(p.id)+'">'+esc(p.name||p.id)+'</option>'}).join('')||'<option value="">没有可用图片 API</option>';
        renderModels();
    }
    async function loadBase(){
        var data=await api('/api/ecommerce-agent/platforms');
        state.platforms=data.platforms||[];
        state.providers=(data.providers||[]).filter(function(p){return p.enabled!==false&&(p.image_models||[]).length});
        state.brands=[];
        var templateData=await api('/api/ecommerce-agent/detail-templates');
    ...
    function renderHistory(){
        ...
        target.innerHTML=state.history.length?state.history.slice(0,12).map(function(run){
            ...
            return '<button type="button" class="ec-history-item'+selected+'" data-ec-history-id="'+esc(run.id)+'"><span><strong>'+esc(name)+'</strong><small>'+esc(dateText(run.created_at))+'</small></span><span class="ec-history-meta"><b>'+esc(statusLabel(run.status))+'</b><small>'+Number(run.success_count||0)+' / '+Number(run.total_images||0)+' 张</small></span></button>';
    ...
    async function parseRequest(){
        try{
            var data=await api('/api/ecommerce-agent/parse-request',{method:'POST',body:JSON.stringify({request_text:one('#ecRequest').value,platforms:platformIds()})});
            var q=data.quantity||{};
             one('#ecParseSummary').innerHTML='<span class="ec-chip">图片 <strong>'+Number(q.total_images||0)+' 张</strong></span><span class="ec-chip">任务批次 <strong>'+Number((q.batches||[]).length||1)+'</strong></span>';
    ...
    function updateRun(){
        ...
        one('#ecRunStatus').innerHTML='<div class="ec-status-top"><strong>'+esc(statusLabel(run.status))+'</strong><span>'+esc(run.current_stage||'')+'</span></div><div class="ec-progress" style="--progress:'+Number(run.progress||0)+'%"><i></i></div><div class="ec-metrics"><div><b>'+Number(run.total_images||0)+'</b><span>总张数</span></div><div><b>'+Number(run.success_count||0)+'</b><span>成功</span></div><div><b>'+Number(run.failed_count||0)+'</b><span>失败</span></div><div><b>'+Number(run.retry_count||0)+'</b><span>重做</span></div></div>'+(run.warnings||[]).map(function(w){return '<div class="ec-warning">'+esc(w)+'</div>'}).join('');
        var outputs=run.outputs||[];
        one('#ecResultCount').textContent=outputs.filter(function(o){return o.status==='succeeded'}).length+' / '+Number(run.total_images||0)+' 张';
        one('#ecResults').innerHTML=outputs.length?outputs.map(function(o){
            if(o.url)return '<div class="ec-result" role="button" tabindex="0" data-ec-view-output="'+esc(o.id)+'"><img src="'+esc(o.url)+'" loading="lazy" alt="生成结果"><em>'+esc((o.plan&&((o.plan.module_name)||o.plan.purpose))||'创作结果')+'</em>'+(o.cost&&o.cost.amount!=null?'<small>¥'+Number(o.cost.amount).toFixed(4)+'</small>':'')+'</div>';
        ...
        one('#ecPause').disabled=terminal||run.status==='paused';one('#ecResume').disabled=terminal||run.status!=='paused';one('#ecCancel').disabled=terminal;one('#ecFactOnly').disabled=!(run.inferred_claims||[]).length;one('#ecExport').disabled=!terminal;one('#ecViewDetail').disabled=!outputs.some(function(o){return o.status==='succeeded'&&o.url});
    ...
    function loadViewerImage(){
        var img=one('#ecViewerImage'),stateEl=one('#ecViewerState');if(!img||!stateEl)return;
        clearTimeout(state.viewerTimer);var token=++state.viewerLoadToken;var src=state.viewerSources[state.viewerSourceIndex];
        img.style.display='none';img.removeAttribute('src');stateEl.className='ec-viewer-state loading';stateEl.textContent=state.viewerSourceIndex?'正在切换备用预览...':'正在加载图片...';
    ...
    function showViewer(index){
        ...
        var box=one('#ecDetailViewer');if(!box){document.body.insertAdjacentHTML('beforeend','<div id="ecDetailViewer" class="ec-detail-viewer" role="dialog" aria-modal="true"><div class="ec-viewer-panel"><button type="button" class="ec-viewer-close" id="ecViewerClose">×</button><div class="ec-viewer-kicker">详情页结果预览</div><h3 id="ecViewerTitle"></h3><div class="ec-viewer-image-wrap"><div id="ecViewerState" class="ec-viewer-state loading">正在加载图片...</div><img id="ecViewerImage" alt="详情页结果"></div><div class="ec-viewer-meta" id="ecViewerMeta"></div><div class="ec-viewer-actions"><button type="button" class="ec-btn" id="ecViewerPrev">上一张</button><button type="button" class="ec-btn" id="ecViewerNext">下一张</button><a class="ec-btn primary" id="ecViewerDownload" download>下载图片</a></div></div></div>');box=one('#ecDetailViewer');one('#ecViewerClose').onclick=function(){clearTimeout(state.viewerTimer);box.classList.remove('open')};one('#ecViewerPrev').onclick=function(){showViewer(state.viewerIndex-1)};one('#ecViewerNext').onclick=function(){showViewer(state.viewerIndex+1)};box.onclick=function(e){if(e.target===box){clearTimeout(state.viewerTimer);box.classList.remove('open')}}}
        one('#ecViewerTitle').textContent=(o.plan&&((o.plan.module_name)||o.plan.purpose))||'详情页结果';one('#ecViewerDownload').href=o.url;one('#ecViewerDownload').download='详情页-'+String((o.plan&&o.plan.index)||state.viewerIndex+1).padStart(2,'0')+'.png';one('#ecViewerMeta').textContent='第 '+String((o.plan&&o.plan.index)||state.viewerIndex+1)+' 张 · '+((state.run.template_name)||'创作结果')+(o.cost&&o.cost.amount!=null?' · 费用 ¥'+Number(o.cost.amount).toFixed(4):'');state.viewerSources=viewerSources(o.url);state.viewerSourceIndex=0;box.classList.add('open');loadViewerImage();
    }
    ...
    function bind(){
        ...
        one('#ecTemplatePlan').addEventListener('click',function(e){if(e.target.closest('#ecApplyTemplate')){one('#ecRequest').value='按当前详情页模板生成每个模块，保持商品主体和原有文字不变';var id=state.detailPlan&&state.detailPlan.template_id;if(id)selectTemplate(id);else parseRequest();}});
        ...
        one('#ecProvider').onchange=renderModels;one('#ecRequest').oninput=function(){clearTimeout(state.parseTimer);state.parseTimer=setTimeout(parseRequest,350)};
    ...
    function init(){
        if(!bridge())return;document.body.insertAdjacentHTML('beforeend',html());bind();selectedPreview();loadBase().then(function(){return loadHistory(true)}).catch(function(e){one('#ecWarnings').innerHTML='<div class="ec-warning">'+esc(e.message)+'</div>'});if(window.lucide)lucide.createIcons();window.EcommerceAgentUI={open:open,close:close,refresh:selectedPreview};
    }
