...
GITHUB_RAW_ROOT = "https://raw.githubusercontent.com/cqiqi271/xiaoqi-ai-canvas/main"
...
MODELSCOPE_RAW_ROOT = "https://www.modelscope.cn/studios/xtwlgzs/Infinite-Canvas/raw/master"
...
LINGJING_DEFAULT_BASE_URL = "https://api.lvziai.xyz"
...
def read_project_config() -> Dict[str, str]:
    defaults = {
        ...
        "mirror_tree_url": "",
        "mirror_raw_root_url": "",
        "mirror_repo_url": MODELSCOPE_REPO_URL,
    }
    try:
...
def app_info():
    version = current_app_version()
    cfg = read_project_config()
    repo_url = cfg.get("repo_url") or cfg.get("home_url") or GITHUB_REPO_URL
    ...
    return {
        ...
        "home_url": cfg.get("home_url") or "/static/project-home.html",
        "repo_url": repo_url,
        "version_url": github_version_url,
        "tree_url": github_tree_url,
        "sources": {
            "github": {
                "label": "自定义更新源" if cfg.get("version_url") else "GitHub",
                "repo_url": repo_url,
                "version_url": github_version_url,
                "tree_url": github_tree_url,
                "update_notes_url": github_notes_url,
...
def github_json(url: str, use_etag_cache: bool = False):
    ...
    cache_key = url
    primary_tree_url = read_project_config().get("tree_url") or GITHUB_TREE_URL
    if use_etag_cache and cache_key == primary_tree_url:
...
def modelscope_update_file_list() -> List[str]:
    ...
    resp = github_get(read_project_config().get("mirror_tree_url") or MODELSCOPE_TREE_URL, headers={"User-Agent": "Infinite-Canvas-Updater"}, timeout=30)
    payload = json.loads(resp.content.decode("utf-8", errors="replace"))
...
def github_update_file_list() -> Tuple[List[str], List[str], List[str]]:
    tree_data = github_json(read_project_config().get("tree_url") or GITHUB_TREE_URL, use_etag_cache=True)
